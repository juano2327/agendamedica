			...
			<section id='secfrm1'>
				...
				<section id='secfrm2C'>
					...
					<?php 
						try
						{	$clase='filaa';
							$Query 	="SELECT Cod_Usu,Id_Usu,Cod_Cita,Fecha_Cita,Hora_Cita,Lugar_Cita FROM agenda.citas WHERE Cod_Usu='$txtCodigo'";
							$rs		=$conn->Execute($Query);
							...
							while (!$rs->EOF)
								...
								echo "<tr class='$clase'>";
								echo "<td>{$rs->fields['Id_Usu']}</td>";
								echo "<td>{$rs->fields['Fecha_Cita']}</td>";
